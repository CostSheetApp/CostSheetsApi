...
module.exports = function(Costsheethasmaterials) {
    ...
    var sql = 'select Mat.costSheetId ' +
			  '		   ,sum(Mat.Total) TotalMaterial ' +
			  '	  from ( ' +
    ...
    Costsheethasmaterials.remoteMethod
  (
    'TotalMaterial',
      {
        http: {path: '/:id/TotalMaterial', verb: 'get'},
    ...
    var sql = 'select  sheet.Id ' +
			  ...
			  '		   ,mat.description ' +
			  '		   ,unit.abbreviation ' +
			  '		   ,unit.description unitsofmeasurement ' +
			  '		   ,ifnull(hist.cost, 0) cost ' +
			  '		   ,ifnull(hist.waste, 0) waste ' +
			  '		   ,ifnull(hist.performance, 0) performance ' +
			  '		   ,(ifnull(hist.cost, 0) * ifnull(hist.waste, 0) * ifnull(hist.performance, 0)) Total ' +
			  '	  from costsheets.costsheethasmaterials as sheet  ' +
			  '		   inner join costsheets.material as mat ' +
			  '				   on mat.id = sheet.materialId ' +
  ...
  (
    'DetailsMaterials',
      {
        http: {path: '/:id/DetailsMaterials', verb: 'get'},
